    ...
    <div className="space-y-5">
      ...
      <section className="rounded border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
        ...
        {forecast && forecast.upcoming.length === 0 && (
          <p className="text-sm text-slate-400">No upcoming charges 🎉</p>
        )}
        ...
        {forecast && forecast.upcoming.length > 0 && (
          <div className="mt-3 border-t border-slate-100 pt-3 text-sm dark:border-slate-800">
            <div className="text-slate-500 dark:text-slate-400">
              Still expected: {euros(Math.abs(forecast.expected_expense_cents))} out, {euros(forecast.expected_income_cents)} in
            </div>
            {report && (
              <div className="mt-1 font-medium">
                Projected month net:{' '}
                {(() => {
                  const projected = report.net_cents + forecast.expected_income_cents + forecast.expected_expense_cents;
                  return <span className={projected < 0 ? 'text-red-600' : 'text-green-700'}>{euros(projected)}</span>;
                })()}
              </div>
            )}
          </div>
